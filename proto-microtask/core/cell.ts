...
//
// Scheduling: the VALUE GRAPH stays synchronous and pull-based — a write
// drains the backward queue to a fixpoint before returning, so read-back
// and lens laws hold synchronously everywhere. Only EFFECTS (terminal
// sinks) defer to a microtask, which makes every synchronous run an
// implicit batch (effects coalesce; net-zero writes are suppressed by
// `checkDirty` at flush time) — so explicit `batch()` is unneeded.
...
// `_bwd !== undefined`.
//
// The `stateful` payload (complement machinery of a complement-carrying
// lens) hangs off a named field so it stays distinctly typed; it is rare,
// so a plain 1→1 / multi-out lens leaves it `undefined` and pays only
// `parent` + `put` + `queueIdx`.
class BwdSpec {
  ...
  // call when neither moved — the backward analog of forward's "Pending
  // but not Dirty ⇒ don't recompute". `readsSource` marks the 2-arg form
  // (1-arg/source-independent puts key on `target` alone). Only the
  // single-parent `buildLens1` path participates; split/stateful keep
...
Object.defineProperty(Cell.prototype, "value", {
  ...
  set(this: Cell<unknown>, next: unknown): void {
    ...
    // prunes its no-op without committing.
...
//
// Walk up `_bwdParent`, applying `put` at each lens (or splitting across N
// parents for a multi-out lens), until a source is committed via the
// forward write path. The walk stops early at a concrete no-op (the parent
// already holds the pushed value). Not a second engine: every path
...
//                      commits are synchronous everywhere.
//   • runEffects()   — terminal effects, drained on a MICROTASK. Coalesces
//                      multiple writes' notifications into one tick. An
//                      effect that writes commits its value graph
//                      synchronously (its own flush) and queues further
//                      effects, so the loop reaches a fixpoint across
//                      however many microtasks it takes.
