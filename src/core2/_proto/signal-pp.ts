// Symmetric bidirectional reactive engine — push-pull backward variant.
// Forward is alien-signals verbatim; a backward write PUSHES (stash target +
// enqueue, O(1)) and is PULLED lazily — the put-chain resolves when an affected
// source is first read or at flush, coalescing repeated writes / net-zero
// reverts for free. A cell's role is set purely by which fields are present;
// writable iff `_bwd !== undefined`. `pendingValue` is dual-keyed (staged
// forward write for a source, deferred backward target for a getter).
