...
// `_queueIdx`); the flush loop alternates bwd-drain / effect-drain to a
// fixpoint.
...
/** Backward worklist: lens cells with deferred writes. Drained to a
 *  fixpoint with effects by flush. */
...
class BwdSpec {
  ...
  /** Relation-member write-through: the cell's forward view is a SOLVED
   *  projection, not its write target (the `base` parent). The generic
   *  view-equality no-op guard is therefore wrong here — it would compare
   *  against the solved value (and trigger a solve) instead of the base — so
   *  the setter skips it and lets the base's own write prune no-ops. */
  member = false;
...
Object.defineProperty(Cell.prototype, "value", {
  ...
  set(this: Cell<unknown>, next: unknown): void {
    ...
    // coalesce (last-write-wins); eager + synchronous otherwise.
    const b = this._bwd;
    if (b === undefined) {
    ...
    if (deferred && (Array.isArray(b.parent) || b.stateful !== undefined)) {
      // Multi-parent / stateful: defer to flush so a split coalesces and a
      // complement steps once. Reuse `pendingValue` (unused by a getter's
      // forward path) as the deferred backward target; drained by flush.
      // Entry no-op vs the current view (GetPut) skips the walk.
    ...
    if (!deferred && !b.member && this._equals(next, this.peek())) return;
...
function propagateBwd(start: Cell<unknown>, target: unknown, deferred: boolean): void {
  ...
  while (true) {
    ...
    // the current source from `put`. `settled` reads a batched source's
 ...
 *  lens/multi-parent re-enters the backward pass. Always called under a
 ...
 *  `m` dirty and notifies existing subscribers so derived views re-pull. */
export function becomeMember<T>(m: Cell<T>, project: () => T, base: Cell<T>): void {
  ...
  b.member = true; // skip the solved-view no-op guard (compare base instead)
  m.flags = F.Mutable | F.Dirty;
  if (m.subs !== undefined) propagate(m.subs, false);
}
