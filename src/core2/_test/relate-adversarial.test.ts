...
import { batch, effect, num, range } from "../index";
...
describe("batch", () => {
  it("writes inside a batch coalesce; reads after see the solved fixpoint", () => {
    const a = span(0, 100);
    const b = span(0, 100);
    equal(a, b);
    batch(() => {
      a.value = { lo: 10, hi: 100 };
      a.value = { lo: 20, hi: 90 }; // last-write-wins on the base
    });
...
// ── 13. mid-batch read reflects the staged base ─────────────────────

describe("mid-batch read", () => {
  it("reading a member inside the batch sees the just-written base", () => {
    const a = span(0, 100);
    const b = span(0, 100);
    equal(a, b);
    let mid = { lo: 0, hi: 0 };
