// Symmetric bidirectional reactive engine (prototype). Forward is
// alien-signals verbatim; backward compiles a view-edit into source-edits by
// walking up `_bwd.parent` and committing through the same forward write path.
// A cell's role is set purely by which fields are present; writable iff
// `_bwd !== undefined`. `pendingValue` is dual-keyed (staged forward write for
// a source, deferred backward target for a getter). Outside a batch writes are
// eager+synchronous; inside, lens writes coalesce last-write-wins and merge
// folds defer until all contributors land.
...
/** Backward worklist: deferred lens writes + merge cells awaiting fold. */
...
// Backward sidecar: lives off a single `_bwd` pointer (not inline on `Cell`)
// so a source/computed stays lean — the forward hot path never touches it and a
// plain node drops ~64 B. `merge` / `stateful` are rare named payloads left
// `undefined` for a plain lens.
class BwdSpec {
...
// Module-level so the polymorphic-`this` statics can call them; each `new Cls()`
// yields the right subclass (so `Vec.lens(...)` returns a `Vec`).
...
// Stateful lens: carries a COMPLEMENT — memory the source can't hold. The four
// handlers are pure (evaluated speculatively by the equality check) and read no
// cells; the engine owns `c`, advancing it only on a real recompute/commit and
// stepping it to the current sources before `bwd` runs.
//   init(srcs)              → seed the complement
//   fwd(srcs, c)            → the view
//   step(srcs, c, external) → advance the complement; `external` = outside
//                             change vs own back-write
//   bwd(target, srcs, c)    → { updates (undefined ⇒ leave parent), complement }
...
// Bare (untyped) factories, inferring `R` from the closures; the
// polymorphic-`this` statics are for typed subclasses like `Vec.lens`.
...
// `flush` alternates backward-drain and effect-drain to a fixpoint (effects may
// write, queuing more of either).
...
// A watching node whose body fires when any SUBSCRIBED dep changes but whose own
// writes self-exclude it (via `activeNetwork`) so it doesn't re-trigger itself.
// Topology is explicit (deps array + subscribe/unsubscribe; body reads add no
// deps) — the building block for constraint networks vs auto-tracked effects.
