...
// structural fact, gated by the `BackLinked` flag so it links exactly once. A
// forward recompute (`_update`) that wipes `BackLinked` makes every later
// back-write re-push a duplicate — an unbounded `_lensSubs` + per-tick-time
// leak that only shows up under sustained write→read churn. Guard it directly.
// ════════════════════════════════════════════════════════════════════════
...
describe("no reverse-edge leak under sustained churn", () => {
  ...
  it("observed chain (forward recompute each tick) does not re-link duplicates", () => {
    ...
    for (let i = 1; i <= 300; i++) {
      (top as { value: number }).value = i;
      void top.value; // forward recompute every tick — the BackLinked-wipe trigger
    }
