...
### 6.2 UI-coupled moves (don't generalise — bireactive has no tree)
...
> required has already been paid in bireactive — for a different reason
...
# Part III — Implications for bireactive
...
- **Animator runtime** (`animation/anim.ts`) — time-driven, frame-clock,
  long-lived gens; animations and choreography.
- **Reactive runtime** (`core/...`) — event-driven, microtask-flushed,
...
**P1 — Two-phase effect.** Concrete shape of the API in bireactive. Can
...
**P5 — `refresh(cell)` as a primitive.** Precise meaning given bireactive
...
- **JS generators** — the color-removal mechanism (and bireactive has them).
...
bireactive's symmetric-lens module — for a different reason.

Whether the assembly is worth doing is, as the main document says of
the bireactive runtime itself, *an empirical question* — does it
